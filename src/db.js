...
function normalizePlaceName(value) {
  return String(value || '').normalize('NFKD').replace(/[\u0300-\u036f]/g, '').toLocaleLowerCase('en').replace(/[ßẞ]/g, 'ss').replace(/[æǽ]/g, 'ae').replace(/œ/g, 'oe').replace(/[øǿ]/g, 'o').replace(/ł/g, 'l').replace(/[đð]/g, 'd').replace(/þ/g, 'th').replace(/[^\p{L}\p{N}]+/gu, '');
}

function openDatabase(filename = process.env.DB_PATH || path.join(process.cwd(), 'data', 'maplens.sqlite')) {
  fs.mkdirSync(path.dirname(filename), { recursive: true });
  const db = new Database(filename);
  db.function('normalize_place_name', { deterministic: true }, normalizePlaceName);
  ...
  db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS city_names (city_id INTEGER NOT NULL REFERENCES cities(id) ON DELETE CASCADE, name TEXT NOT NULL COLLATE NOCASE, search_name TEXT, PRIMARY KEY(city_id,name));
...
function seedGeo(db) {
  ...
  const alias=db.prepare('INSERT OR IGNORE INTO city_names(city_id,name,search_name) VALUES (?,?,?)'),aliases=[[1850147,'Tokio']];
  db.transaction(()=>{continents.forEach(x=>a.run(...x));countries.forEach(x=>b.run(...x));cities.forEach(x=>{c.run(...x);alias.run(x[0],x[2],normalizePlaceName(x[2]))});aliases.forEach(x=>alias.run(...x,normalizePlaceName(x[1])))})();
}
module.exports={openDatabase,normalizePlaceName};
